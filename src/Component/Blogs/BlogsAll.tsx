...
const BlogsAll: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 mt-20 flex flex-col">
      ...
      <div className="max-w-6xl mx-auto grid gap-8 sm:grid-cols-2 lg:grid-cols-3 flex-1">
  {listBlogs.map((item, index) => (
    <div
      key={index}
      className="bg-white shadow-lg rounded-xl p-6 flex flex-col"
  ...
  <h2
    className="text-lg sm:text-xl font-semibold text-slate-900 leading-snug truncate"
    style={{ fontFamily: "DM Sans" }}
    title={item.blogTitle} // shows full title on hover
  >
    {item.blogTitle}
  </h2>
  <p
    className="text-xs sm:text-sm text-gray-500 flex-shrink-0 whitespace-nowrap"
    style={{ fontFamily: "DM Sans" }}
  >
    {new Date(item.blogDate).toLocaleDateString("en-US", {
      month: "long",
      ...
      <div className="flex justify-end w-full mt-4">
        <span
          onClick={() =>
            navigate(
              `/blog-view/${item.blogId}/${encodeURIComponent(item.blogTitle)}`,
              { state: item }
            )
          }
          className="text-[#fdbe1b] font-medium cursor-pointer underline"
          style={{ fontFamily: "DM Sans" }}
        >
          Read More
        </span>
